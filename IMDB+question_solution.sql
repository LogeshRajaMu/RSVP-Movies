...
sum(case when worldwide_gross_income is NULL then 1 else 0 end) as worldwide_gross_income_null_count,
...
select count(id) as Number_of_Movies_produced_in_USA_or_India_in_2019 from movie
...
select genre, 
count(movie_id) as Total_Num_of_Movies 
from genre g 
join movie m on g.movie_id = m.id 
group by genre 
order by Total_Num_of_Movies 
desc limit 1;
...
with movies_belonging_to_one_genre as (select movie_id from genre 
group by movie_id 
having count(distinct genre) = 1) 
select count(*) as movies_belonging_to_one_genre 
from movies_belonging_to_one_genre;
...
dense_rank() over (order by count(movie_id) 
desc) genre_rank 
...
dense_rank() 
over(order by count(id) desc) prod_company_rank 
from movie m 
join ratings r 
on m.id = r.movie_id
...
case
    ...
    end as 'German_Movies_Are_Popular_Than_Italian_Movies'
from german_case
inner join
italian_case
using(unique_id);
...
with top_3_directors as (select genre, 
count(m.id) as movie_count, 
dense_rank() over(order by count(m.id) desc) 
as genre_rank from movie m
inner join genre g 
on g.movie_id = m.id
inner join ratings as r 
on r.movie_id = m.id 
...
inner join top_3_directors using (genre)
...
count(rm.movie_id) as movie_count from role_mapping rm
...
with top_3_genres as( select genre, count(movie_id) as movie_count, rank() over(order by 
count(movie_id) desc) genre_rank from genre
group by genre limit 3 ),

find_rank as (select genre, year, title as movie_name, worldwide_gross_income, rank() over
(order by worldwide_gross_income desc) as movie_rank from movie as m 
inner join genre as g on m.id=g.movie_id 
where genre in (select genre from top_3_genres)
)
select * from find_rank 
where movie_rank <= 5 limit 5;
...
top_director_summary as ( select *, datediff(next_date_published, date_published) as date_difference 
